...
def get_tfidf(analyzer: str):
    ...
    if joblib_path and os.path.exists(joblib_path):
        ...
        if all(p and os.path.exists(p) for p in matrix_paths.values()):
            ...
            print(f"    Matrices loaded in {time.time() - t0:.2f} seconds")
        else:
            print(">>> Transforming data (this may take a while)...")
            t0 = time.time()
            Xtr = vec.transform(X_train_text)
            print(f"    Train transform: {time.time() - t0:.2f} seconds")

            t1 = time.time()
            Xva = vec.transform(X_val_text)
            print(f"    Val transform: {time.time() - t1:.2f} seconds")

            t2 = time.time()
            Xte = vec.transform(X_test_text)
            print(f"    Test transform: {time.time() - t2:.2f} seconds")

            if args.cache_dir:
                print(">>> Caching transformed matrices...")
                sparse.save_npz(matrix_paths["train"], Xtr)
                sparse.save_npz(matrix_paths["val"], Xva)
                sparse.save_npz(matrix_paths["test"], Xte)

            print(f"    All transforms completed in {time.time() - t0:.2f} seconds")
    ...
    t1 = time.time()
    Xva = vec.transform(X_val_text)
    print(f"    Transform val: {time.time() - t1:.2f} seconds")

    t2 = time.time()
    Xte = vec.transform(X_test_text)
    print(f"    Transform test: {time.time() - t2:.2f} seconds")

    print(f"    Total fit+transform: {time.time() - fit_start:.2f} seconds")
